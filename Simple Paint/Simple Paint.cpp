...
WCHAR* fileNameSaved = NULL;
WCHAR fileDir[MAX_LOADSTRING];
WCHAR openFileDir[MAX_LOADSTRING];

int fileNameSavedLength = 0;
HPEN hPen;
PAINTSTRUCT ps;
...
int CaptureAnImage(HWND hWnd)
{
    ...
    wcscat(fileNameSaved, L".bmp");
    wcscpy(fileDir, L"Saved images//");
    wcscat(fileDir, fileNameSaved);
...
bool LoadAndBlitBitmap(HBITMAP hBitmap)
{
    HDC hWinDC = BeginPaint(hwnd, &ps);
...
void OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    case ID_FILE_OPEN:
    {
        ...
        ofn.lpstrFilter = L"Images (*.bmp)\0*.bmp\0\0";
        ofn.nFilterIndex = 1;
    ...
    case ID_EDIT_REDO:
    {
        if (cacheRedoTypes.size() > 0) {
            onUndoRedo = true;
            if (cacheRedoTypes.back() == "Shape") {
                if (cacheShapes.size() > 0) {
                    shapes.push_back(cacheShapes.back());
                    cacheShapes.erase(cacheShapes.begin() + cacheShapes.size() - 1);
                ...
                if (cacheTexts.size() > 0) {
                    ...
                    cacheTexts.erase(cacheTexts.begin() + cacheTexts.size() - 1);
                    cacheTypes.push_back("Text");
                }
            }
            cacheRedoTypes.erase(cacheRedoTypes.begin() + cacheRedoTypes.size() - 1);
...
INT_PTR CALLBACK UserSave(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == SAVEBOX_CANCEL)
        {
            ...
            WCHAR* buffer = new WCHAR[len + 1];
            GetWindowText(inputItem, buffer, len + 1);
            fileNameSaved = buffer;
            fileNameSavedLength = len;
            EndDialog(hDlg, LOWORD(wParam));
            isSave = true;
            return (INT_PTR)TRUE;
        }
